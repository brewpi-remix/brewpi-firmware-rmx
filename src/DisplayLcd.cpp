...
LcdDriver LcdDisplay::lcd(0x27, 20, 4);  // 20x4 LCD, address will autodetect
#else
...
void LcdDisplay::printAllTemperatures(void)
{
	...
	if (flags & LCD_FLAG_ALTERNATE_ROOM)
	{
		...
		if (displayRoom ^ ((flags & LCD_FLAG_DISPLAY_ROOM) != 0))
		{ // transition
...
{
	printTemperatureAt(6, 2, flags & LCD_FLAG_DISPLAY_ROOM ? tempControl.ambientSensor->read() : tempControl.getFridgeTemp());
}
...
void LcdDisplay::printState(void)
{
	...
	if (state != stateOnDisplay)
	{ //only print static text when state has changed
