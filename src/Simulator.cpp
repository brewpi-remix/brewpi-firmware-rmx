...
{
#if BREWPI_EMULATE
    // In the avr simulator (we call emulator to try to distinguish), ticks
    // take forever. 1 second takes many minutes if emulating waiting for
    // millis() to increment.
...
void simulateLoop(void)
{
    ...
    // only needed if we want the arduino to be self running. Useful for manual
    // testing, but not so much with an external driver.
    updateSimulationTicks();

    if (ticks.millis() - lastUpdate >= (1000))
...
void HandleSimulatorConfig(const char *key, const char *val, void *pv)
{
    ...
    else if (strcmp_P(key, SimulatorDoorState) == 0)
    { // 0 for closed, anything else for open
    ...
    else if (strcmp_P(key, SimulatorEnabled) == 0)
    { // 0 for closed, anything else for open
